import React from 'react'

const FooterLinks = () => {
  return (
    <div>
       <div>
      <div className="container w-full max-w-6xl px-5 md:px-10 mx-auto mt-20 md:mt-40">
        <div className="space-y-10 leading-relaxed divide-white md:space-y-0 md:grid md:grid-cols-4 md:divide-x divide-opacity-5">
          <div className="text-center">
            <div className="text-xl font-medium">About</div>
            <div className="mt-2 leading-loose text-secondary">
              <ul>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>About Aurora</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>News &amp; Updates</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Press Kit</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Videos</span>
                  </a>
                </li>
              </ul>
            </div>
          </div>
          <div className="text-center">
            <div className="text-xl font-medium">Ecosystem</div>
            <div className="mt-2 leading-loose text-secondary">
              <ul>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Ecosystem Apps</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Tokens</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Partner Program</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Aurora DAO</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>AURORA Token</span>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Faucet</span>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Rainbow Bridge</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
          </div>
          <div className="text-center">
            <div className="text-xl font-medium">Developers</div>
            <div className="mt-2 leading-loose text-secondary">
              <ul>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Developer Portal</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Documentation</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Block Explorer</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Faucet</span>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Testnet Bridge</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Audits</span>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>AuditOne Bug Bounty</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>HackenProof Bug Bounty</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Discord</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>GitHub</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
          </div>
          <div className="text-center">
            <div className="text-xl font-medium">Support</div>
            <div className="mt-2 leading-loose text-secondary">
              <ul>
                <li>
                  <a
                    className="text-left hover:text-white md:text-center group"
                    href="#"
                  >
                    <span>Get Help</span>
                  </a>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Knowledge Base</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
                <li>
                  <button className="text-left hover:text-white md:text-center group">
                    <span>Contact</span>
                  </button>
                </li>
                <li>
                  <a
                    href="#"
                    target="_blank"
                    rel="noreferrer"
                    className="text-left hover:text-white md:text-center pl-3 group"
                  >
                    <span>Discord</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                      className="inline-block w-3 h-3 ml-1 -rotate-45 opacity-50 group-hover:opacity-100"
                    >
                      <path
                        fillRule="evenodd"
                        d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                        clipRule="evenodd"
                      ></path>
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap justify-center gap-10 mx-auto mt-20 text-sm text-center sm:max-w-lg">
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 448 512"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    d="M297.216 243.2c0 15.616-11.52 28.416-26.112 28.416-14.336 0-26.112-12.8-26.112-28.416s11.52-28.416 26.112-28.416c14.592 0 26.112 12.8 26.112 28.416zm-119.552-28.416c-14.592 0-26.112 12.8-26.112 28.416s11.776 28.416 26.112 28.416c14.592 0 26.112-12.8 26.112-28.416.256-15.616-11.52-28.416-26.112-28.416zM448 52.736V512c-64.494-56.994-43.868-38.128-118.784-107.776l13.568 47.36H52.48C23.552 451.584 0 428.032 0 398.848V52.736C0 23.552 23.552 0 52.48 0h343.04C424.448 0 448 23.552 448 52.736zm-72.96 242.688c0-82.432-36.864-149.248-36.864-149.248-36.864-27.648-71.936-26.88-71.936-26.88l-3.584 4.096c43.52 13.312 63.744 32.512 63.744 32.512-60.811-33.329-132.244-33.335-191.232-7.424-9.472 4.352-15.104 7.424-15.104 7.424s21.248-20.224 67.328-33.536l-2.56-3.072s-35.072-.768-71.936 26.88c0 0-36.864 66.816-36.864 149.248 0 0 21.504 37.12 78.08 38.912 0 0 9.472-11.52 17.152-21.248-32.512-9.728-44.8-30.208-44.8-30.208 3.766 2.636 9.976 6.053 10.496 6.4 43.21 24.198 104.588 32.126 159.744 8.96 8.96-3.328 18.944-8.192 29.44-15.104 0 0-12.8 20.992-46.336 30.464 7.68 9.728 16.896 20.736 16.896 20.736 56.576-1.792 78.336-38.912 78.336-38.912z"
                    fill="currentColor"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">Discord</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  viewBox="0 0 34 35"
                  xmlns="http://www.w3.org/2000/svg"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    fill="currentColor"
                    d="M0 25.1v-7.8c0-4.7 1.8-8.8 5.2-12.1 2.6-2.5 5.8-4 9.4-4.5C21.5-.4 28.3 3 31.8 9.1c1.6 3 2.4 6.2 2.2 9.5-.4 4.6-2.3 8.5-5.7 11.6-2.5 2.2-5.4 3.6-8.6 4.1-.6.1-1.3.2-2 .2H.6c-.4 0-.5-.1-.5-.5-.1-3-.1-6-.1-8.9zM16.9 7.8c-.6 0-1.2.1-1.8.2-6.2 1.3-9.6 8.1-6.7 13.7.1.3.1.5.1.7-.5 1.6-1 3.2-1.4 4.7-.1.1-.1.3-.1.4.1.1.3 0 .4-.1 1.7-.4 3.4-.9 5.1-1.3.3-.1.6-.1.9.1 1.6.7 3.4.9 5.1.6 5.2-.8 8.9-5.7 8.1-11-.8-4.5-4.8-8-9.7-8z"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">Forum</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 496 512"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    fill="currentColor"
                    d="M248 8C111 8 0 119 0 256s111 248 248 248 248-111 248-248S385 8 248 8zm121.8 169.9-40.7 191.8c-3 13.6-11.1 16.9-22.4 10.5l-62-45.7-29.9 28.8c-3.3 3.3-6.1 6.1-12.5 6.1l4.4-63.1 114.9-103.8c5-4.4-1.1-6.9-7.7-2.5l-142 89.4-61.2-19.1c-13.3-4.2-13.6-13.3 2.8-19.7l239.1-92.2c11.1-4 20.8 2.7 17.2 19.5z"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">Telegram</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  strokeWidth="0"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    d="M8 2H1l8.26 11.014L1.45 22H4.1l6.388-7.349L16 22h7l-8.608-11.478L21.8 2h-2.65l-5.986 6.886L8 2Zm9 18L5 4h2l12 16h-2Z"
                    stroke="none"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">X</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 576 512"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    fill="currentColor"
                    d="M549.655 124.083c-6.281-23.65-24.787-42.276-48.284-48.597C458.781 64 288 64 288 64S117.22 64 74.629 75.486c-23.497 6.322-42.003 24.947-48.284 48.597-11.412 42.867-11.412 132.305-11.412 132.305s0 89.438 11.412 132.305c6.281 23.65 24.787 41.5 48.284 47.821C117.22 448 288 448 288 448s170.78 0 213.371-11.486c23.497-6.321 42.003-24.171 48.284-47.821 11.412-42.867 11.412-132.305 11.412-132.305s0-89.438-11.412-132.305zm-317.51 213.508V175.185l142.739 81.205-142.739 81.201z"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">YouTube</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  viewBox="0 0 36 36"
                  fill="currentColor"
                  xmlns="http://www.w3.org/2000/svg"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path d="M18 0C8.06 0 0 8.058 0 18c0 7.953 5.157 14.7 12.31 17.08.9.165 1.23-.39 1.23-.868 0-.427-.017-1.56-.025-3.06-5.007 1.086-6.064-2.415-6.064-2.415-.818-2.078-1.999-2.632-1.999-2.632-1.633-1.118.125-1.094.125-1.094 1.806.127 2.757 1.855 2.757 1.855 1.605 2.75 4.213 1.956 5.238 1.495.163-1.163.628-1.956 1.143-2.406-3.997-.454-8.199-1.999-8.199-8.896 0-1.966.701-3.572 1.852-4.83-.185-.456-.803-2.285.177-4.764 0 0 1.511-.484 4.95 1.845 1.436-.4 2.975-.599 4.506-.606 1.528.007 3.068.207 4.507.606 3.436-2.329 4.944-1.845 4.944-1.845.983 2.479.365 4.308.179 4.764 1.154 1.258 1.85 2.864 1.85 4.83 0 6.915-4.208 8.436-8.218 8.882.646.556 1.222 1.654 1.222 3.333 0 2.407-.022 4.348-.022 4.938 0 .482.324 1.041 1.238.866C30.847 32.693 36 25.95 36 18c0-9.942-8.06-18-18-18"></path>
                </svg>
              </div>
              <div className="mt-2">GitHub</div>
            </a>
          </div>
          <div className="flex flex-col items-center justify-center">
            <a href="#" target="_blank" rel="noreferrer" className="text-white">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-translucent-darker hover:backdrop-brightness-200 md:w-20 md:h-20">
                <svg
                  viewBox="0 0 29 20"
                  xmlns="http://www.w3.org/2000/svg"
                  role="img"
                  width="32"
                  height="32"
                  className="fill-current"
                >
                  <path
                    d="M9.554 17.509 2 10l7.554-7.509M19.536 2.491 27 10l-7.464 7.509"
                    stroke="currentColor"
                    strokeWidth="3"
                    strokeLinecap="round"
                    fill="none"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </div>
              <div className="mt-2">Near.Social</div>
            </a>
          </div>
        </div>
      </div>
      <div className="text-white !mt-0 flex flex-col justify-center items-center pb-4 h-96 relative">
        <div className="flex items-center justify-center flex-1 mb-8">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 324 342"
            role="img"
            className="w-32 h-32"
          >
            <path
              d="M126.47 242.92a.37.37 0 0 1 .37.38v15.95c0 7.57-4.73 11.29-11 11.29s-11-3.72-11-11.29V243.3a.38.38 0 0 1 .38-.38H109a.38.38 0 0 1 .37.38V259c0 5.28 2.76 7.37 6.52 7.37s6.51-2.09 6.51-7.37v-15.7a.38.38 0 0 1 .38-.38zm64.99 13.58a14.08 14.08 0 1 1-14.08-14 13.83 13.83 0 0 1 14.08 14zm-23.74 0a9.64 9.64 0 1 0 9.66-9.85 9.51 9.51 0 0 0-9.66 9.85zM230 270.08h-3.84a.38.38 0 0 1-.35-.52L236 243.32a.55.55 0 0 1 .53-.36h4.8a.57.57 0 0 1 .53.36L252 269.56a.38.38 0 0 1-.35.52h-3.84a.39.39 0 0 1-.35-.25l-8.19-21.6a.37.37 0 0 0-.7 0l-8.2 21.6a.37.37 0 0 1-.37.25zm-153.78 0h-3.84a.38.38 0 0 1-.35-.52l10.15-26.24a.57.57 0 0 1 .53-.36h4.8a.55.55 0 0 1 .53.36l10.16 26.24a.38.38 0 0 1-.35.52H94a.37.37 0 0 1-.35-.25l-8.19-21.6a.38.38 0 0 0-.71 0l-8.19 21.6a.39.39 0 0 1-.34.25zm142.48-.61-7.62-9.74c4.3-.81 6.82-3.72 6.82-8.17 0-5.11-3.33-8.6-9.22-8.6h-10.63a.57.57 0 0 0-.57.57 3.62 3.62 0 0 0 3.62 3.61h7.11c3.57 0 5.27 1.83 5.27 4.46s-1.66 4.49-5.27 4.49h-10.16a.58.58 0 0 0-.57.57v13a.38.38 0 0 0 .38.38h3.63a.38.38 0 0 0 .37-.38V260H206l6.62 8.63a3.77 3.77 0 0 0 3 1.48h2.75a.38.38 0 0 0 .33-.64zm-61.51 0-7.62-9.74c4.3-.81 6.82-3.72 6.82-8.17 0-5.11-3.33-8.6-9.22-8.6h-10.63a.57.57 0 0 0-.57.57 3.62 3.62 0 0 0 3.62 3.61h7.11c3.57 0 5.27 1.83 5.27 4.46s-1.66 4.49-5.27 4.49h-10.16a.58.58 0 0 0-.57.57v13a.38.38 0 0 0 .38.38H140a.38.38 0 0 0 .37-.38V260h4.19l6.61 8.63a3.77 3.77 0 0 0 3 1.48h2.75a.38.38 0 0 0 .27-.64z"
              fill="currentColor"
            ></path>
            <path
              d="M162 83a15.92 15.92 0 0 1 14.31 8.84l45 90A16 16 0 0 1 207 205h-90a16 16 0 0 1-14.31-23.16l45-90A15.92 15.92 0 0 1 162 83m0-11a27 27 0 0 0-24.15 14.92l-45 90A27 27 0 0 0 117 216h90a27 27 0 0 0 24.15-39.08l-45-90A27 27 0 0 0 162 72z"
              fill="#70d44b"
            ></path>
          </svg>
        </div>
        <div className="flex flex-col mt-auto space-y-3 text-sm text-center sm:space-x-4 sm:block text-secondary sm:space-y-0">
          <span>&copy; Aurora</span>
          <a className="hover:text-white" href="index.html">
            Privacy Policy
          </a>
          <a className="hover:text-white" href="index.html">
            Terms of Use
          </a>
        </div>
      </div>
    </div>
    </div>
  )
}

export default FooterLinks
